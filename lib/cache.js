...
class DependencyCache {
	...
	/**
	 * Prune cache entries where the file's mtime has changed.
	 * @return {Promise}
	 */
	async pruneStaleEntries() {
		...
		for (const filePath in entries) {
			try {
				const stats = await stat(filePath);
				if (stats.mtimeMs !== entries[filePath].mtime) {
					log('cache stale for %s (mtime changed)', filePath);
					toDelete.push(filePath);
				}
			} catch (err) {
				// File no longer exists or can't be accessed
				log('cache entry removed for %s: %s', filePath, err.message);
				toDelete.push(filePath);
			}
		}
		...
		if (toDelete.length > 0) {
			...
			this.dirty = true;
			log('pruned %d stale cache entries', toDelete.length);
		}
	}
	...
	/**
	 * Get cached dependencies for a file (cache already validated on load).
	 * @param {String} filePath - Absolute path to the file
	 * @return {Array|null} - Array of dependencies or null if not cached
	 */
	get(filePath) {
		if (!this.cache) {
			return null;
	 ...
	 */
	set(filePath, dependencies, mtime) {
		if (!this.cache) {
			return;
		}
