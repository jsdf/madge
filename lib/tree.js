...
class Tree {
	...
	generateTree(files) {
		...
		// Pre-populate visited with cached entries
		if (this.cache.isEnabled()) {
			this.populateVisitedFromCache(visited);
		}
		...
		// Update cache with newly discovered dependencies
		if (this.cache.isEnabled()) {
			this.updateCacheFromVisited(visited);
		}

		let tree = this.convertTree(visited, {}, pathCache, npmPaths);
	...
	/**
	 * Populate visited object from cache entries.
	 * Builds nested tree structure that dependency-tree expects.
	 * Only includes files where all dependencies are also in cache.
	 * @param {Object} visited
	 */
	populateVisitedFromCache(visited) {
		const entries = this.cache.cache.entries;
		...
		const buildNestedTree = (file, building) => {
			...
			if (building.has(file)) {
				// Return a marker that we'll resolve later
				return {__circular__: file};
			}
			building.add(file);
			...
			// Build dependency trees first to check if all deps are valid
			const depTrees = {};
			for (const dep of entry.dependencies) {
				const depTree = buildNestedTree(dep, building);
				if (depTree === null) {
					// A dependency is missing from cache, invalidate this file too
					return null;
				}
				depTrees[dep] = depTree;
			}

			// All dependencies are valid, populate visited
			visited[file] = depTrees;
			return visited[file];
		};

		// First pass: build all trees
		...
		const resolveCircular = (obj) => {
			for (const key in obj) {
				if (obj[key] && obj[key].__circular__) {
					// Replace marker with actual reference
					obj[key] = visited[obj[key].__circular__] || {};
				} else if (typeof obj[key] === 'object' && !resolved.has(obj[key])) {
		...
		for (const file in visited) {
			resolved.add(visited[file]);
			resolveCircular(visited[file]);
		}
	}

	/**
	 * Update cache from visited object after dependency discovery.
	 * @param {Object} visited
	 */
	updateCacheFromVisited(visited) {
		for (const file in visited) {
			// Skip if already in cache (mtime already validated)
			...
			try {
				...
				const deps = Object.keys(visited[file]);
				this.cache.set(file, deps, stats.mtimeMs);
			} catch (err) {
				log('failed to cache file %s: %s', file, err.message);
			}
		}
	}
